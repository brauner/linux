...
static void bm_kill_super(struct super_block *sb)
{
...
static int bm_fill_super(struct super_block *sb, struct fs_context *fc)
{
	 ...
	 * ->enabled again in ->kill_sb() as someone might already be mounting
	 * binfmt_misc again. It also would be pointless since by the time
	 * ->kill_sb() is called we know that the binary type list for this
...
/*
 * This will set fc->s_fs_info to user_ns. If this fails in bm_fill_super()
 * after we allocated a new superblock then ->kill_sb() will take care of
 * put_user_ns(). In any other case this will be taken care of in bm_free().
 */
