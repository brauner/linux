...
static inline bool coredump_force_suid_safe(const struct coredump_params *cprm)
{
	...
	return cprm->dumpable == SUID_DUMP_ROOT;
}
				 ...
				 const struct linux_binfmt *binfmt)
{
	...
	if (!binfmt->core_dump)
		return true;
	if (cprm->dumpable == SUID_DUMP_DISABLE)
		return true;
	return false;
}
...
void vfs_coredump(const kernel_siginfo_t *siginfo)
{
	...
	struct coredump_params cprm = {
		 ...
		 * by any locks.
		 *
		 * Note that we only care about MMF_DUMP_FILTER_* bits here;
		 * dumpability has moved to task->exec_state and is snapshotted
		 * separately below.
		 */
