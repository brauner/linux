...
static struct kmem_cache *f_owner_cache __ro_after_init;
...
int file_f_owner_allocate(struct file *file)
{
	...
	f_owner = kmem_cache_zalloc(f_owner_cache, GFP_KERNEL);
	if (!f_owner)
		return -ENOMEM;
	...
	if (unlikely(!try_cmpxchg(&file->f_owner, NULL, f_owner)))
		kmem_cache_free(f_owner_cache, f_owner);
	return 0;
}
...
static int __init fcntl_init(void)
{
	...
	f_owner_cache = kmem_cache_create("file_owner_cache",
					  sizeof(struct fown_struct), 0,
					  SLAB_PANIC | SLAB_ACCOUNT, NULL);
	return 0;
}
