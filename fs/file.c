...
/* Slots needed to cover every open descriptor without close-on-exec set. */
static unsigned int inheritable_fdtable_size(struct fdtable *fdt)
{
	unsigned int i = fdt_words(fdt);

	while (i--) {
		unsigned long word = fdt->open_fds[i] & ~fdt->close_on_exec[i];

		if (word)
			return i * BITS_PER_LONG + __fls(word) + 1;
	}
	return 0;
}

/* Slots needed to cover every open descriptor inside @range. */
static unsigned int fd_range_fdtable_size(struct fdtable *fdt, struct fd_range *range)
{
	unsigned int size = min(range->to, last_fd(fdt)) + 1;
	unsigned int last = find_last_bit(fdt->open_fds, size);

	if (last == size || last < range->from)
		return 0;
	return last + 1;
}
 ...
 *
 * DUP_FD_CLOEXEC_EXCEPT turns punch_hole around. What survives is every
 * descriptor without close-on-exec set plus whatever the range covers, so
 * the table only has to reach the last of those. It requires punch_hole.
 */
				      ...
				      unsigned int flags)
{
	...
	if (flags & DUP_FD_CLOEXEC_EXCEPT) {
		unsigned int size = max(inheritable_fdtable_size(fdt),
					fd_range_fdtable_size(fdt, punch_hole));

		if (!size)
			return NR_OPEN_DEFAULT;
		return ALIGN(size, BITS_PER_LONG);
	}

	last = find_last_bit(fdt->open_fds, fdt->max_fds);
	if (last == fdt->max_fds)
		return NR_OPEN_DEFAULT;
...
static bool fd_is_dropped(unsigned int fd, struct fdtable *fdt,
			  struct fd_range *punch_hole, unsigned int flags)
{
	if (!(flags & DUP_FD_CLOEXEC_EXCEPT))
		return false;
	if (fd >= punch_hole->from && fd <= punch_hole->to)
		return false;
 ...
 * on failure.  For 'punch_hole' and 'flags' see sane_fdtable_size().
 */
			    ...
			    unsigned int flags)
{
	...
	open_files = sane_fdtable_size(old_fdt, punch_hole, flags);
	...
	for (fd = 0; fd < open_files; fd++) {
		...
		if (f && !fd_is_dropped(fd, old_fdt, punch_hole, flags)) {
			get_file(f);
		} else {
			f = NULL;
...
static inline unsigned int next_fd_to_close(struct fdtable *fdt, unsigned int fd,
					    unsigned int max_fd, bool cloexec)
{
	if (cloexec)
				 ...
				 unsigned int max_fd, bool cloexec)
{
	...
	for (fd = next_fd_to_close(fdt, fd, max_fd, cloexec);
	     fd <= max_fd;
	     fd = next_fd_to_close(fdt, fd + 1, max_fd, cloexec)) {
		file = file_close_fd_locked(files, fd);
		if (file) {
			spin_unlock(&files->file_lock);
			filp_close(file, files);
...
/* Close every open close-on-exec descriptor outside of [fd, max_fd]. */
static inline void __range_close_cloexec_except(struct files_struct *files,
						unsigned int fd, unsigned int max_fd)
{
	if (fd > 0)
		__range_close(files, 0, fd - 1, true);
	if (max_fd < UINT_MAX)
		__range_close(files, max_fd + 1, UINT_MAX, true);
}
 ...
 *
 * With CLOSE_RANGE_CLOEXEC_EXCEPT the range says what to keep instead.
 * Every open file descriptor with close-on-exec set is closed unless it
 * falls into [@fd, @max_fd]. To keep nothing, name a range that cannot
 * hold an open file descriptor, e.g. close_range(~0U, ~0U, ...).
 */
		...
		unsigned int, flags)
{
	...
	/*
	 * One asks for file descriptors to be marked close-on-exec, the other
	 * acts on the ones already marked. They aren't the same request.
	 */
	if ((flags & CLOSE_RANGE_CLOEXEC) && (flags & CLOSE_RANGE_CLOEXEC_EXCEPT))
		return -EINVAL;
	...
	if ((flags & CLOSE_RANGE_UNSHARE) && atomic_read(&cur_fds->count) > 1) {
		...
		if (flags & CLOSE_RANGE_CLOEXEC)
			punch_hole = NULL;
		else if (flags & CLOSE_RANGE_CLOEXEC_EXCEPT)
			dup_flags = DUP_FD_CLOEXEC_EXCEPT;

		fds = dup_fd(cur_fds, punch_hole, dup_flags);
		if (IS_ERR(fds))
			return PTR_ERR(fds);
		/*
	...
	if (flags & CLOSE_RANGE_CLOEXEC) {
		...
		/*
		 * If we unshared above, dup_fd() never carried the
		 * close-on-exec descriptors over in the first place.
		 */
		if (!fds)
			__range_close_cloexec_except(cur_fds, fd, max_fd);
	} else {
		__range_close(cur_fds, fd, max_fd, false);
	}
