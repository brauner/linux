		   ...
		   struct dentry *new)
{
	...
	for (name = buf; list_size; name += slen) {
		...
		if (ovl_is_private_xattr(sb, name))
			continue;
retry:
		size = vfs_getxattr(old_user_ns, old, name, value, value_size);
		if (size == -ERANGE)
			size = vfs_getxattr(old_user_ns, old, name, NULL, 0);
		...
		error = vfs_setxattr(new_user_ns, new, name, value, size, 0);
		if (error) {
			...
			struct dentry *upperdentry, struct kstat *stat)
{
	...
	return notify_change(user_ns, upperdentry, &attr, NULL);
}
		 ...
		 struct kstat *stat)
{
	...
	if (!S_ISLNK(stat->mode)) {
		...
		err = notify_change(user_ns, upperdentry, &attr, NULL);
	}
	if (!err) {
		struct iattr attr = {
			    ...
			    struct dentry *upper)
{
	...
	index = lookup_one_len(user_ns, name.name, indexdir, name.len);
	if (IS_ERR(index)) {
		err = PTR_ERR(index);
	} else {
...
static int ovl_link_up(struct ovl_copy_up_ctx *c)
{
	...
	upper = lookup_one_len(c->user_ns, c->dentry->d_name.name, upperdir,
			       c->dentry->d_name.len);
	err = PTR_ERR(upper);
	if (!IS_ERR(upper)) {
...
static int ovl_copy_up_workdir(struct ovl_copy_up_ctx *c)
{
	...
	upper = lookup_one_len(c->user_ns, c->destname.name, c->destdir, c->destname.len);
	err = PTR_ERR(upper);
	if (IS_ERR(upper))
		goto cleanup;
			    ...
			    struct dentry *dentry, char *name, char **value)
{
	...
	res = vfs_getxattr(user_ns, dentry, name, NULL, 0);
	...
	if (res > 0) {
		...
		res = vfs_getxattr(user_ns, dentry, name, buf, res);
		if (res < 0)
			kfree(buf);
		else
			*value = buf;
...
static int ovl_copy_up_meta_inode_data(struct ovl_copy_up_ctx *c)
{
	...
	if (capability) {
		err = vfs_setxattr(user_ns, upperpath.dentry, XATTR_NAME_CAPS,
