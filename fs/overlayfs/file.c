				      ...
				      struct inode *realinode)
{
	...
	err = inode_permission(user_ns, realinode, MAY_OPEN | acc_mode);
	if (err) {
		realfile = ERR_PTR(err);
	} else if (!inode_owner_or_capable(user_ns, realinode)) {
				...
				unsigned long arg, unsigned int flags)
{
	...
	if (!inode_owner_or_capable(mnt_user_ns(file->f_path.mnt), inode))
		return -EACCES;
