...
{
	struct dentry *ret = lookup_one_len_unlocked(mnt_user_ns, name, base, len);
...
struct dentry *ovl_get_index_fh(struct ovl_fs *ofs, struct ovl_fh *fh)
{
	...
	index = lookup_positive_unlocked(ovl_upper_mnt_user_ns(ofs), name.name,
...
bool ovl_lower_positive(struct dentry *dentry)
{
	...
	for (i = 0; !done && !positive && i < poe->numlower; i++) {
		...
		this = lookup_positive_unlocked(user_ns, name->name, lowerdir,
						name->len);
		if (IS_ERR(this)) {
			switch (PTR_ERR(this)) {
			case -ENOENT:
