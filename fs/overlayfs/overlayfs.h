...
{
	int err = vfs_rmdir(user_ns, dir, dentry);
...
static inline int ovl_do_unlink(struct user_namespace *user_ns,
				struct inode *dir, struct dentry *dentry)
{
	int err = vfs_unlink(user_ns, dir, dentry, NULL);
...
{
	int err = vfs_link(old_dentry, user_ns, dir, new_dentry, NULL);
...
static inline int ovl_do_create(struct user_namespace *user_ns,
				...
				umode_t mode)
{
	int err = vfs_create(user_ns, dir, dentry, mode, true);
...
static inline int ovl_do_mkdir(struct user_namespace *user_ns,
			       ...
			       umode_t mode)
{
	int err = vfs_mkdir(user_ns, dir, dentry, mode);
...
{
	int err = vfs_mknod(user_ns, dir, dentry, mode, dev);
...
static inline int ovl_do_symlink(struct user_namespace *user_ns,
				 ...
				 const char *oldname)
{
	int err = vfs_symlink(user_ns, dir, dentry, oldname);
				      ...
				      void *value, size_t size)
{
	...
	return vfs_getxattr(user_ns, dentry, name, value, size);
}
				  ...
				  size_t size)
{
	...
	int err = vfs_setxattr(user_ns, dentry, name, value, size, 0);
				     ...
				     enum ovl_xattr ox)
{
	const char *name = ovl_xattr(ofs, ox);
	struct user_namespace *user_ns = ovl_upper_mnt_user_ns(ofs);
	int err = vfs_removexattr(user_ns, dentry, name);
...
{
	struct dentry *ret = vfs_tmpfile(user_ns, dentry, mode, 0);
