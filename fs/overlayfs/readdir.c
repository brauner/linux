			       ...
			       struct dentry *dir, struct ovl_readdir_data *rdd)
{
	...
	if (!err) {
		while (rdd->first_maybe_whiteout) {
			...
			dentry = lookup_one_len(user_ns, p->name, dir, p->len);
			if (!IS_ERR(dentry)) {
...

{
	...
	this = lookup_one_len(mnt_user_ns(path->mnt), p->name, dir, p->len);
			   ...
			   struct list_head *list)
{
	...
	list_for_each_entry(p, list, l_node) {
		...
		dentry = lookup_one_len(user_ns, p->name, upper, p->len);
		if (IS_ERR(dentry)) {
...
static int ovl_workdir_cleanup_recurse(struct path *path, int level)
{
	...
	list_for_each_entry(p, &list, l_node) {
		...
		dentry = lookup_one_len(mnt_user_ns(path->mnt), p->name,
					path->dentry, p->len);
		if (IS_ERR(dentry))
			continue;
		if (dentry->d_inode)
...
int ovl_indexdir_cleanup(struct ovl_fs *ofs)
{
	...
	list_for_each_entry(p, &list, l_node) {
		...
		index = lookup_one_len(user_ns, p->name, indexdir, p->len);
		if (IS_ERR(index)) {
			err = PTR_ERR(index);
			index = NULL;
			break;
