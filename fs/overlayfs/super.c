...
retry:
	work = lookup_one_len(user_ns, name, ofs->workbasedir, strlen(name));
	...
	if (!IS_ERR(work)) {
		 ...
		 */
		err = vfs_removexattr(user_ns, work, XATTR_NAME_POSIX_ACL_DEFAULT);
		if (err && err != -ENODATA && err != -EOPNOTSUPP)
			goto out_dput;

		err = vfs_removexattr(user_ns, work, XATTR_NAME_POSIX_ACL_ACCESS);
		if (err && err != -ENODATA && err != -EOPNOTSUPP)
			goto out_dput;

		/* Clear any inherited mode bits */
		inode_lock(work->d_inode);
		err = notify_change(user_ns, work, &attr, NULL);
				     ...
				     struct dentry *workdir)
{
	...
	whiteout = lookup_one_len(user_ns, name.name.name, workdir, name.name.len);
					   ...
					   const char *name, umode_t mode)
{
	...
	child = lookup_one_len(user_ns, name, parent, len);
