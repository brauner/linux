...
void pidfs_add_pid(struct pid *pid)
{
	...
	/*
	 * On 32bit pidfs_ino() allows us to notice when we wrap around
	 * UINT_MAX in the lower 32bit. We then increment the 64bit
	 * number by two to start the lower 32bit with inode number 2
	 * again.
	 *
	 * On 32bit the upper 32bit are used as the inode generation
	 * number. Userspace can reconstruct the 64bit unique identifier
	 * by combining the inode number and the generation number.
	 */
