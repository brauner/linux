 ...
 */
static bool grab_super(struct super_block *s)
{
	if (super_lock_excl(s) && atomic_inc_not_zero(&s->s_active))
		return true;
	super_unlock_excl(s);
	return false;
}
...
__releases(&bdev->bd_holder_lock)
{
	...
	WARN(!sb, "Missing superblock while thawing!");
