					...
					struct bpf_map *map, int func_id)
{
	...
	case BPF_MAP_TYPE_INODE_STORAGE:
		if (func_id != BPF_FUNC_inode_storage_get &&
			...
			goto error;
		break;
	case BPF_MAP_TYPE_BDEV_STORAGE:
...
#ifdef CONFIG_BPF_LSM
BTF_ID(func, bpf_bdev_storage_get)
#else
BTF_ID_UNUSED
#endif
			       ...
			       const struct btf_type *ptr_type, struct btf *desc_btf)
{
	...
	if (meta->func_id == special_kfunc_list[KF_bpf_obj_new_impl] ||
		 ...
		 */
	} else if (meta->func_id == special_kfunc_list[KF_bpf_bdev_storage_get]) {
		struct bpf_map *map = meta->map.ptr;

		if (!map) {
			verbose(env, "bpf_bdev_storage_get: map pointer not found\n");
			return -EINVAL;
		}
		if (map->map_type != BPF_MAP_TYPE_BDEV_STORAGE) {
			verbose(env, "bpf_bdev_storage_get requires BDEV_STORAGE map, got %d\n",
				map->map_type);
			return -EINVAL;
		}
		mark_reg_known_zero(env, regs, BPF_REG_0);
